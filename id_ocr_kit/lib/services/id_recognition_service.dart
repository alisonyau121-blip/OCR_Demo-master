...
class IdRecognitionService {
  ...
  Future<IdRecognitionResult> recognizeId(File imageFile) async {
    try {
      // Step 1: Perform OCR
      // Dual-script recognition: Latin (for numbers/MRZ) + Chinese (for names/addresses)
      final ocrResult = await _ocrProvider.recognizeText(
        OcrRequest(
          imageFile: imageFile,
      ...
      final parsedIds = IdParser.parseAll(ocrResult.text);
      ...
      return IdRecognitionResult.error(
        error: e.message,
        errorType: _mapOcrErrorType(e.type),
      );
    } catch (e) {
      return IdRecognitionResult.error(
        error: 'Unexpected error: $e',
        errorType: IdRecognitionErrorType.unexpectedError,
      );
    }
  }
  ...
  Future<void> dispose() => _ocrProvider.dispose();
  
  /// Maps OCR error types to recognition error types
  IdRecognitionErrorType _mapOcrErrorType(OcrErrorType ocrError) {
    switch (ocrError) {
      case OcrErrorType.fileNotFound:
        return IdRecognitionErrorType.fileNotFound;
      case OcrErrorType.invalidFormat:
        return IdRecognitionErrorType.invalidImageFormat;
      case OcrErrorType.processingFailed:
      case OcrErrorType.unsupportedScript:
      case OcrErrorType.notInitialized:
        return IdRecognitionErrorType.ocrFailed;
    }
  }
}
