...
/// - Insert signatures into form field positions
/// - Resource management
class SyncfusionPdfAdapter implements PdfProvider {
  ...
  /// 
  /// This method finds a form field by name and inserts the signature
  /// image at that field's position, maintaining aspect ratio.
  Future<void> insertSignatureAtFormField({
    required PdfDocument document,
    ...
    try {
      _log.info('Inserting signature at form field: $fieldName');
      
      final form = doc.form;
      bool found = false;
      
      // DEBUG: List all form fields
      _log.info('Total form fields in PDF: ${form.fields.count}');
      for (int i = 0; i < form.fields.count; i++) {
        final field = form.fields[i];
        _log.info('Field $i: name="${field.name}", type=${field.runtimeType}');
      }
      ...
      for (int i = 0; i < form.fields.count; i++) {
        ...
        if (field.name == fieldName) {
          _log.info('Found field "$fieldName" at index $i');
          ...
          // Create bitmap from signature PNG bytes
          final image = sf.PdfBitmap(signatureBytes);
          
          // Calculate aspect ratio to maintain proportions
          final imageAspect = image.width / image.height;
          final fieldAspect = bounds.width / bounds.height;
      ...
      if (!found) {
        throw PdfException(
          'Form field "$fieldName" not found in PDF',
      ...
      _log.severe('Failed to insert signature at form field', e, st);
      throw PdfException(
        'Failed to insert signature: $e',
        PdfErrorType.insertFailed,
        e,
      );
    }
