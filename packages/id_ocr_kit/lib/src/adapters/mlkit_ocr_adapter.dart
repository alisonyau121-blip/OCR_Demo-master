...
class MlKitOcrAdapter implements OcrProvider {
  ...
  TextRecognizer _initializeRecognizer(TextRecognitionScript script) {
    try {
      ...
      // TEMPORARY FIX: Only Latin script is bundled by default
      // Chinese/Japanese/Korean require additional ML Kit model downloads
      // For MVP, force Latin-only to prevent crashes
      if (script != TextRecognitionScript.latin) {
        _log.warning('Non-Latin script requested but not available. Using Latin fallback.');
        _failedScripts.add(script);
        throw OcrException(
          'Script $script not available. Only Latin script is bundled.',
          OcrErrorType.unsupportedScript,
        );
      }
